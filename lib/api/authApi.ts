...
const AuthApi = {
  AppleSignIn: async ({
    ...
    try {
      ...
      if (response.status === 200 && response.data.status === 200) {
        ...
        throw new Error(`🚨 Apple Sign-In failed: ${JSON.stringify(response.data)}`);
      }
    } catch (error: any) {
  ...
  mergeApplePhone: async ({
    apple_uid,
    apple_email,
    phone_uid,
    phone_number, // pass as +819099999999 string
  }: {
    apple_uid: string;
    apple_email: string;
    phone_uid: string;
    ...
    try {
      const response = await ApiClient.post('/merge_apple_phone', {
        apple_uid,
        apple_email,
        phone_uid,
        phone_number, 
      ...
      if (response.status === 200 && response.data.status === 200) {
        ...
        throw new Error(`🚨 Merge failed: ${JSON.stringify(response.data)}`);
      }
    } catch (error: any) {
      console.log(`${error.message}`)
      throw new Error(`Merge Error: ${error.message}`);
    }
  },
